...
```{r Analysis Cleaning working with NA, echo = FALSE}
# counts the number of NAs per column, resulting in:
colna <- colSums(is.na(arrhythmia))
colna <- colna[colna > 0]
colna %>% knitr::kable("html", caption = 'Table of Ocurrence of NA', col.names = c( 'Ocurrences')) %>%
...
```{r Correlation creating the tidy table, echo=FALSE}
...
cor_arrhythmia_numeric %>% slice_max(order_by = value, n = 10) %>% knitr::kable("html", caption = "Top 10 Correlation between predictors")
```
...
```{r Correlation with limit, echo=FALSE}
limit <- 0.9
cor_arrhythmia_numeric %>% filter(value < -limit | value > limit) %>% count() %>% knitr::kable("html", caption = "Correlation over 0.9 & under -0.9") %>%
  kable_styling(full_width = F, position = "left")
```
...
```{r Correlation of previous missing values P Vector angles, echo=FALSE}

cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "P Vector angles" | correlated.2 %in% "P Vector angles") %>% slice_max(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for P Vector angles") %>%
  kable_styling(full_width = F, position = "left")
cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "P Vector angles" | correlated.2 %in% "P Vector angles") %>% slice_min(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for P Vector angles") %>%
  kable_styling(full_width = F, position = "left")
```
...
```{r Correlation of T Vector angles, echo=FALSE}

cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "T Vector angles" | correlated.2 %in% "T Vector angles") %>% slice_max(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for T Vector angles") %>%
  kable_styling(full_width = F, position = "left")
cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "T Vector angles" | correlated.2 %in% "T Vector angles") %>% slice_min(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for T Vector angles") %>%
  kable_styling(full_width = F, position = "left")
```
...
```{r Correlation of QRST Vector angles,echo=FALSE}

cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "QRST Vector angles" | correlated.2 %in% "QRST Vector angles") %>% slice_max(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for QRST Vector angles") %>%
  kable_styling(full_width = F, position = "left")
cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "QRST Vector angles" | correlated.2 %in% "QRST Vector angles") %>% slice_min(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for QRST Vector angles") %>%
  kable_styling(full_width = F, position = "left")
```
...
```{r Correlation Heart rate, echo=FALSE }

cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "Heart rate" | correlated.2 %in% "Heart rate") %>% slice_max(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for Heart rate") %>%
  kable_styling(full_width = F, position = "left")
cor_arrhythmia_numeric %>%  filter( correlated.1 %in% "Heart rate" | correlated.2 %in% "Heart rate") %>% slice_min(order_by = value, n = 5) %>% knitr::kable("html", caption = "Correlation for Heart rate") %>%
  kable_styling(full_width = F, position = "left")
```
...
#### Less Correlation
Less correlated predictors are also presented and they are:
```{r Correlation Less Correlated, echo=FALSE}
limit <- 0.0001
cor_arrhythmia_numeric %>% filter(value > -limit & value < limit)  %>% knitr::kable("html", caption = "Less Correlated Preditors") %>%
  kable_styling(full_width = F, position = "left")
```
...
arrhythmia %>% ggplot(aes(Age)) + geom_boxplot() + ggtitle("Age Boxplot")
```
...
arrhythmia %>% ggplot(aes(Height)) + geom_boxplot() + ggtitle("Height Boxplot")
```
...
arrhythmia %>% ggplot(aes(Weight)) + geom_boxplot() + ggtitle("Weight Boxplot")
```
...
```{r  Create train set check train_set, echo=FALSE, message=FALSE}
left_join(train_set, Class, by = "Class code") %>% 
  ...
  knitr::kable("html", caption = "Presence of codes in train_set dataset")
  
```

## check validation_set dataset

```{r  Create train set check validation_set, echo=FALSE, message=FALSE}
left_join(validation_set, Class, by = "Class code") %>% 
  group_by(`Class code`, `Class name`) %>% summarise( "N Ocurrences" = n(), Percentage = round(100*n()/nrow(arrhythmia),digits = 1)) %>%
  knitr::kable("html", caption = "Presence of codes in validation_set dataset")
