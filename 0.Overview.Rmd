...
The Arrhythmia dataset is a table witch each row it is collection of health measurement for each patience and the last one says if this persons it is a normal health or has same of the 15 arrhythmia.
...
left_join(arrhythmia, Class, by = "Class code") %>% group_by(`Class code`, Class) %>% summarise( n = n(), Percentage = 100*n()/nrow(arrhythmia))
```

You can see than some arrytmia type are not present in our dataset (11,12 and 13).
...
left_join(arrhythmia, Class, by = "Class code") %>% ggplot(aes(Class)) + geom_histogram(stat="count") +
  ...
  ylab("Number of Patiences") +
  ggtitle("Distribution of Cardiac Arrhythmia")
```
...
1.- Start analyzing the dataset, cleaning, working on missing values that we found, review predictors that does not add value and define the outcomes that we are going to predict.
...
setting properly and analyze the better option to get a good predictor.
