...
### Second Predictions
Let's works with the second Prediction, that include all the classes:  
...
These are predictors are present in the tree too, not all of them because the tree shows less than 20 predictors, and the overall does not show all of them, only 30 are rated, and it is not directly related with the order in the decision tree. Now see how well it is the prediction in the validation set:
...
#rpart.plot(pruned_fit$finalModel, extra = 2, roundint=FALSE,
#  box.palette = list("Gy", "Gn", "Bu", "Bn", "Or", "Rd", "Pu", "RdYlGn", "GnYlRd", "BlGnYl","YlGnBl",
#    "GyGy", "GyGn")) # specify 13 colors)
...
cm <- confusionMatrix(data = y_hat_rf, reference = validation_set_p1$`Class code`) #$overall["Accuracy"]
cm
options(digits =7) # default
```
...
cm <- confusionMatrix(data = y_hat_rf, reference = validation_set$`Class code`) #$overall["Accuracy"]
cm
options(digits =7) # default
```
...
#library(e1071)
set.seed(1)
#train_svm <- svm(`Class code` ~ ., data = train_set_p1)
#train_svm

fit_svm <- train(`Class code` ~ ., data = train_set_p1, method="svmRadial")  # caret train method 
fit_svm

#y_hat_svm <- predict(train_svm,validation_set_p1)
...
#train_svm_tune <- tune(svm, `Class code` ~ .,data = train_set_p1, 
#              kernel="radial", ranges=list(cost=10^(-1:2), gamma=c(.5,1,2)))
#train_svm_tune
#train_svm_after_tune <- svm(`Class code` ~ ., data = train_set_p1, kernel="radial", cost=0.1, gamma=0.5)
#y_hat_svm_tune <- predict(train_svm_after_tune,validation_set_p1)
```
...
#library(e1071)
set.seed(1)
fit_svm <- svm(`Class code` ~ ., data = train_set)
fit_svm
