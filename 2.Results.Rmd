...
Pred1_results[-1,] %>% data.frame(row.names = NULL) %>% knitr::kable("html", caption = "Prediction Normal/Arrhytmia Summary", "html") %>%
...
```{r}
varimp_knn_p2_mean %>% data.frame()  %>% rownames_to_column() %>% filter(rowname == "Channel V1 QRSA")
varimp_svm_p2 %>% data.frame()  %>% rownames_to_column() %>% filter(rowname == "Channel V1 QRSA")
```
This predictor appear only in random forest. Is this something only used for this model? Let's check its correlation:
...
Let's see if in the other models appear
