...
Pred2_results <- data.frame(Model = "Just the most common",
                            ...
                            row.names = NULL)
Pred2_results[,1:11] %>% knitr::kable(caption = "Prediction All Arrhytmia Classification Summary", "html") %>%
add_header_above(header = c(" " = 2, "Class 1" = 3, "Class 2" = 3, "Class 3" = 3))
```
...
Decision Tree is the most intuitive solution that we get from data, and it s very popular en health industry, that the system give you a quick rule to answer your question.
...
options(digits =7) # default
```
We can not any improvement, but the decision tree is similar but not equall under tge change made by prune.
...
#delete
train_svmxx <- svm(`Class code` ~ .,kernel = "polynomial",cost = 1,
                     #tuneGrid = data.frame(ranges = seq(0, 2, len = 10)),
                     data = train_set1)

#svc <- svm(Y~.,data = df_train.boruta,kernel = "linear",cost = 1)
tune.out <- tune(svm,`Class code` ~ .,data = train_set1,ranges = list(cost= c(0.001,0.05,0.01,0.1,1,5,10,100)), kernel = "polynomial")
bestsvc <- tune.out$best.model
summary(bestsvc)
svcpred <- predict(bestsvc,validation_set1)
svcpred1 <- predict(train_svm,validation_set1)
