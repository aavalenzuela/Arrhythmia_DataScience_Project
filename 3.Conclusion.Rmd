...
Some approach artificially expand the size of the dataset predictors by multiplying/averaging/(other function) some observations. This can work in some cases, required knowledge to be made properly. We are not to aplly this here, besides the last point, we already have to many preictors for our algoristh.
...
The initial task of cleaning the data, using this real data, shows its importance, detecting data incomplete, erroneous data, that can lead us to a bad conclusion. More knowledge of physiology can be useful in detect more erroneous data that we do not applied in this works for all the predictors
...
```{r}
Pred2_results[4,] %>% data.frame(row.names = NULL) %>% t() %>% knitr::kable(caption = "Prediction All Arrhytmia Classification Summary", "html") %>%
  kable_styling(full_width = F)
```
...
The original problem was that some software can not detect the arrhythmia properly. With this works we can see we can create some level of confidence in the case of “Normal” & “Arrhythmia” and for all the classes but for a more robust solution in the different kind of arrhythmia more data will be necessary to train our model.
