...
sd0.list.arrhythmia <- names(sd.arrhythmia[sd.arrhythmia == 0 & !is.na(sd.arrhythmia)])

# Indentify the column number to be deleted
...
```{r}
data_numeric <- select_if(arrhythmia2, is.numeric)             # Subset numeric columns with dplyr
#data_numuric
...
test_index <- createDataPartition(y = arrhythmia$`Class code`, times = 1, p = 0.5, list = FALSE)
train_set <- arrhythmia[-test_index,]
validation_set <- arrhythmia[test_index,]

unique(train_set$`Class code`)
unique(validation_set$`Class code`)
dim(train_set)
dim(validation_set)
