...
This show in blue the mean and in green the median for this predictors. We are going to use the median which look more independent of the low values that look lake a outsiders.
...
```{r}
arrhythmia2 %>% filter(is.na(`QRST Vector angles`)) %>% select(Age, Sex, Height, Weight, `QRS duration`,`P-R interval`, `Q-T interval`, `T interval`, `P interval` ,`QRS Vector angles`)
```
...
```{r}
arrhythmia2 %>% filter(is.na(`Heart rate`)) %>% select(Age, Sex, Height, Weight, `QRS duration`,`P-R interval`, `Q-T interval`, `T interval`, `P interval` ,`QRS Vector angles`)
```
Let's do the same analysis about the distribution:
...
```{r Correlation creating the tidy table, include=FALSE}
...
```{r Outliers Weight Table with Age and Height}
...
test_index <- createDataPartition(y = arrhythmia2$`Class code`, times = 1, p = 0.2, list = FALSE)
train_set <- arrhythmia2[-test_index,]
validation_set <- arrhythmia2[test_index,]
...
We are going to use "Class code" as an outcome to predict. Because some outcome are too few as we saw before, we need that they are present in the train_set and validation_set.
...
```{r  xxx, echo=FALSE, message=FALSE}
...
```{r  ddd, echo=FALSE, message=FALSE}
...
sum(values_count  == 1)                                      # Print counts of different values
...
```{r}
train_set1 <- train_set[,values_count > 1]
validation_set1 <- validation_set[,values_count > 1]
```
